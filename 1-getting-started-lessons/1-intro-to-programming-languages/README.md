...
お気に入りのアプリを使用していて、すべてが完璧にクリックされた瞬間をご存知ですか?ボタンをタップすると、「すごい、どうやってそんなことをしたんだろう？」と思うような、本当に魔法のようなことが起こります。そう、おそらくあなたと同じような誰かが、おそらく午前 2 時にお気に入りのコーヒーショップに座って 3 杯目のエスプレッソを飲んでいるのでしょうが、その魔法を生み出すコードを書いたのです。そして、ここであなたの心を驚かせるでしょう。このレッスンが終わる頃には、彼らがどのようにそれを行ったかを理解するだけでなく、自分でも試してみたくなるでしょう。
...
ほら、今プログラミングが怖いと感じているなら、私もよくわかります。私が最初に始めたとき、正直に言って、ある種の数学の天才であるか、5 歳からコーディングをしている必要があると思いました。しかし、ここで私の見方が完全に変わりました。プログラミングは、新しい言語で会話することを学ぶのとまったく同じです。 「こんにちは」と「ありがとう」から始まり、コーヒーを注文するまで、気づけば深い哲学的な議論をしているようです。この場合を除いて、あなたはコンピューターと会話していますが、正直なところ?彼らは、これまでで最も忍耐強い会話パートナーです。決してあなたの間違いを批判せず、やり直すことに常に興奮しています。
