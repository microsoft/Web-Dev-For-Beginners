...
Тази лекция обхваща основите на JavaScript, езикът, който осигурява интерактивност в уеб пространството.
...
✅ Ключовата дума `let` беше въведена в ES6 и предоставя на променливата така наречената _блокова област на действие_. Препоръчително е да използвате `let` вместо `var`. Ще разгледаме блоковите области на действие по-подробно в бъдещи части.
...
1. **Декларирайте променлива**. Нека декларираме променлива, използвайки ключовата дума `let`:
   ...
   `myVariable` вече е декларирана с ключовата дума `let`. В момента няма стойност.
...
1. **Присвояване на стойност**. Съхранете стойност в променлива с оператора `=`, последван от очакваната стойност.
   ...
   > Забележка: използването на `=` в тази лекция означава, че използваме "оператор за присвояване", който се използва за задаване на стойност на променлива. Не означава равенство.
...
1. **Рефакториране**. Заменете кода си със следното изявление.
    ...
    Горният пример се нарича _явна инициализация_, когато променлива се декларира и й се присвоява стойност едновременно.
...
1. **Промяна на стойността на променливата**. Променете стойността на променливата по следния начин:
   ...
   След като променливата е декларирана, можете да промените стойността й по всяко време в кода си с оператора `=` и новата стойност.

   ✅ Опитайте! Можете да пишете JavaScript директно в браузъра си. Отворете прозорец на браузъра и отидете на Developer Tools. В конзолата ще намерите поле за въвеждане; напишете `let myVariable = 123`, натиснете Enter, след това напишете `myVariable`. Какво се случва? Забележете, че ще научите повече за тези концепции в следващите лекции.

## Константи

Декларирането и инициализацията на константа следват същите концепции като променлива, с изключение на ключовата дума `const`. Константите обикновено се декларират с главни букви.
...
- **Трябва да имат стойност**. Константите трябва да бъдат инициализирани, иначе ще възникне грешка при изпълнение на кода.
- **Референцията не може да бъде променена**. Референцията на константа не може да бъде променена след инициализация, иначе ще възникне грешка при изпълнение на кода. Нека разгледаме два примера:
   ...
   - **Референцията на обект е защитена**. Следното НЕ е позволено.
    ...
    - **Стойността на обект не е защитена**. Следното Е позволено:
      ...
      Горният пример показва промяна на стойността на обекта, но не и на самата референция, което го прави позволено.

   > Забележка, `const` означава, че референцията е защитена от преназначаване. Стойността обаче не е _неизменна_ и може да се промени, особено ако е сложна структура като обект.
...
✅ Типовете данни също се наричат JavaScript примитиви, тъй като са най-ниското ниво на типове данни, предоставени от езика. Има 7 примитивни типа данни: string, number, bigint, boolean, undefined, null и symbol. Отделете минута, за да си представите какво може да представлява всеки от тези примитиви. Какво е `zebra`? А `0`? `true`?
...
Има няколко типа оператори за използване при изпълнение на аритметични функции, като някои от тях са изброени тук:
...
**Шаблонни литерали** са друг начин за форматиране на стрингове, освен кавичките, като вместо тях се използва обратна кавичка. Всичко, което не е обикновен текст, трябва да бъде поставено в плейсхолдъри `${ }`. Това включва всякакви променливи, които могат да бъдат стрингове.
...
Можете да постигнете целите си за форматиране с който и да е метод, но шаблонните литерали ще уважават всички интервали и нови редове.

✅ Кога бихте използвали шаблонен литерал вместо обикновен стринг?

### Булеви стойности

Булевите стойности могат да бъдат само две: `true` или `false`. Булевите стойности могат да помогнат при вземане на решения кои редове от кода трябва да се изпълнят, когато са изпълнени определени условия. В много случаи [операторите](../../../../2-js-basics/1-data-types) помагат за задаване на стойността на булева стойност и често ще забележите и пишете променливи, които се инициализират или стойностите им се актуализират с оператор.

- `let myTrueBool = true`
- `let myFalseBool = false`

✅ Променлива може да се счита за 'истинна', ако се оценява като булева стойност `true`. Интересно е, че в JavaScript [всички стойности са истинни, освен ако не са дефинирани като фалшиви](https://developer.mozilla.org/docs/Glossary/Truthy).

---

## 🚀 Предизвикателство

JavaScript е известен със своите изненадващи начини за обработка на типове данни понякога. Направете малко проучване за тези 'изненади'. Например: чувствителността към големи и малки букви може да ви изненада! Опитайте това в конзолата си: `let age = 1; let Age = 2; age == Age` (резултатът е `false` -- защо?). Какви други изненади можете да откриете?
