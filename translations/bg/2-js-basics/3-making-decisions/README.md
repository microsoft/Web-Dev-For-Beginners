...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-27T22:07:23+00:00",
...
[Предварителен тест](https://ashy-river-0debb7803.1.azurestaticapps.net/quiz/11)

Вземането на решения и контролирането на реда, в който се изпълнява вашият код, прави кода ви повторяем и устойчив. Тази секция обхваща синтаксиса за контролиране на потока от данни в JavaScript и неговото значение, когато се използва с булеви типове данни.
...
Булевите стойности могат да имат само две стойности: `true` или `false`. Булевите стойности помагат да се вземат решения кои редове от кода да се изпълнят, когато са изпълнени определени условия.

Задайте булевата стойност като true или false по следния начин:

`let myTrueBool = true`  
`let myFalseBool = false`

✅ Булевите стойности са кръстени на английския математик, философ и логик Джордж Бул (1815–1864).
...
✅ Проверете знанията си, като напишете няколко сравнения в конзолата на браузъра си. Изненадва ли ви някоя от върнатите стойности?

## If израз

If изразът ще изпълни кода между своите блокове, ако условието е вярно.
...
Логическите оператори често се използват за създаване на условието.
...
`else` изразът ще изпълни кода между своите блокове, когато условието е невярно. Той е опционален с `if` израз.
...
`switch` изразът се използва за изпълнение на различни действия въз основа на различни условия. Използвайте `switch` израза, за да изберете един от многото блокове код за изпълнение.
...
| Символ | Описание                                                                                     | Пример                                                                 |
| ------ | -------------------------------------------------------------------------------------------- | --------------------------------------------------------------------- |
| `&&`   | **Логическо И**: Сравнява две булеви изрази. Връща true **само** ако и двете страни са true  | `(5 > 6) && (5 < 6 ) //Едната страна е false, другата е true. Връща false` |
| `\|\|` | **Логическо ИЛИ**: Сравнява две булеви изрази. Връща true, ако поне едната страна е true     | `(5 > 6) \|\| (5 < 6) //Едната страна е false, другата е true. Връща true` |
...
Логическите оператори могат да се използват за създаване на условия в if..else изрази.
...
Досега видяхте как можете да използвате `if...else` израз, за да създадете условна логика. Всичко, което влиза в `if`, трябва да се оценява като true/false. С помощта на оператора `!` можете да _отрицаете_ израза. Това би изглеждало така:
...
Създайте програма, която първо е написана с логически оператори, а след това я пренапишете с тернарен израз. Кой синтаксис предпочитате?
...
Прочетете повече за многото налични оператори [в MDN](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Operators).

Разгледайте чудесния [справочник за оператори](https://joshwcomeau.com/operator-lookup/) на Josh Comeau!
