...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-27T23:02:03+00:00",
...
Ако искате да създадете множество екрани за уеб страница, едно решение би било да създадете отделен HTML файл за всеки екран, който искате да покажете. Въпреки това, този подход има някои неудобства:
...
Ще създадем банково приложение с два екрана: страницата за вход и таблото за управление. Първо, нека добавим в HTML тялото елемент за запазване на място, който ще използваме за създаване на различните екрани на нашето приложение:
...
- Заглавна част с заглавие и връзка за изход
...
Ако опитате текущия HTML файл в браузър, ще видите, че той остава на `Loading...`. Това е така, защото трябва да добавим JavaScript код, за да създадем и покажем HTML шаблоните.
...
Това, което правим тук, са точно 3-те описани стъпки. Създаваме шаблона с `id` `templateId` и поставяме неговото клонирано съдържание в нашия елемент за запазване на място. Забележете, че трябва да използваме `cloneNode(true)`, за да копираме цялото поддърво на шаблона.
...
Ще използваме прост обект, за да реализираме [карта](https://en.wikipedia.org/wiki/Associative_array) между пътищата на URL адресите и нашите шаблони. Добавете този обект в началото на вашия файл `app.js`.
...
Сега нека модифицираме малко функцията `updateRoute`. Вместо директно да предаваме `templateId` като аргумент, искаме първо да погледнем текущия URL адрес и след това да използваме нашата карта, за да получим съответната стойност на `templateId`. Можем да използваме [`window.location.pathname`](https://developer.mozilla.org/docs/Web/API/Location/pathname), за да получим само секцията с пътя от URL адреса.
...
Тук картографирахме маршрутите, които декларирахме, към съответния шаблон. Можете да опитате дали работи правилно, като промените URL адреса ръчно в браузъра си.
...
Ще трябва да използваме JavaScript и по-конкретно [`history.pushState`](https://developer.mozilla.org/docs/Web/API/History/pushState), който позволява обновяване на URL адреса и създаване на нов запис в историята на браузъра, без да се презарежда HTML.

> Забележка: Докато HTML елементът за връзка [`<a href>`](https://developer.mozilla.org/docs/Web/HTML/Element/a) може да се използва самостоятелно за създаване на хипервръзки към различни URL адреси, той по подразбиране ще накара браузъра да презареди HTML. Необходимо е да се предотврати това поведение при обработка на маршрутизация с персонализиран JavaScript, използвайки функцията `preventDefault()` върху събитието за кликване.
...
Ако маршрут не може да бъде намерен, сега ще пренасочим към страницата за вход.
...
Нека завършим системата за навигация, като добавим връзки към нашите *Login* и *Logout* връзки в HTML.
...
✅ Методът `history.pushState` е част от стандарта HTML5 и е имплементиран във [всички модерни браузъри](https://caniuse.com/?search=pushState). Ако създавате уеб приложение за по-стари браузъри, има трик, който можете да използвате вместо този API: използвайки [хеш (`#`)](https://en.wikipedia.org/wiki/URI_fragment) преди пътя, можете да реализирате маршрутизация, която работи с обикновена навигация чрез връзки и не презарежда страницата, тъй като целта му е да създава вътрешни връзки в рамките на страница.
...
![Екранна снимка на историята на навигацията](../../../../translated_images/history.7fdabbafa521e06455b738d3dafa3ff41d3071deae60ead8c7e0844b9ed987d8.bg.png)
...
> Забележка: използвахме [стрелкова функция](https://developer.mozilla.org/docs/Web/JavaScript/Reference/Functions/Arrow_functions), за да декларираме нашия обработчик на събития `popstate` за краткост, но обикновена функция би работила по същия начин.
...
Сега опитайте да използвате бутоните за назад и напред на браузъра си и проверете дали показаният маршрут се обновява правилно този път.
