...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-20T20:29:44+00:00",
...
> **NOTE:** هناك نوع خاص من الدوال يُعرف بـ **الطريقة**، والذي كنت تستخدمه بالفعل! في الواقع، رأينا هذا في العرض التوضيحي أعلاه عندما استخدمنا `console.log`. ما يميز الطريقة عن الدالة هو أن الطريقة مرتبطة بكائن (`console` في مثالنا)، بينما الدالة تكون مستقلة. ستسمع العديد من المطورين يستخدمون هذه المصطلحات بالتبادل.
...
حتى الآن، الدالة التي أنشأناها ستقوم دائمًا بالإخراج إلى [الكونسول](https://developer.mozilla.org/docs/Web/API/console). أحيانًا يكون هذا بالضبط ما نبحث عنه، خاصة عندما ننشئ دوالًا ستستدعي خدمات أخرى. ولكن ماذا لو أردت إنشاء دالة مساعدة لإجراء عملية حسابية وتوفير القيمة مرة أخرى حتى أتمكن من استخدامها في مكان آخر؟
...
1. دالة `add` تأخذ معاملين وتُرجع مجموعهما.
2. دالة `multiply` مع قيم افتراضية للمعاملات (المعامل الثاني افتراضيًا يساوي 1).
3. دالة سهم `square` تأخذ رقمًا وتُرجع مربعه.
4. دالة `calculate` تقبل دالة أخرى كمعامل ورقمين، ثم تطبق الدالة على هذه الأرقام.
