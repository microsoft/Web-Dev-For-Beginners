...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-20T20:38:32+00:00",
...
الطريقة التي نحدد بها جزءًا من التعليمات البرمجية التي نريد تنفيذها هي من خلال إنشاء وظيفة. عندما نفكر في [البرمجة الإجرائية](https://en.wikipedia.org/wiki/Procedural_programming)، يتم استدعاء الوظائف بترتيب معين. نفس الشيء ينطبق على البرمجة القائمة على الأحداث. الفرق هو **كيف** يتم استدعاء الوظائف.

للتعامل مع الأحداث (النقر على الأزرار، الكتابة، إلخ)، نقوم بتسجيل **مستمعي الأحداث**. مستمع الحدث هو وظيفة تستمع لحدوث حدث وتنفذ استجابةً له. يمكن لمستمعي الأحداث تحديث واجهة المستخدم، إجراء مكالمات إلى الخادم، أو أي شيء آخر يحتاج إلى القيام به استجابةً لإجراء المستخدم. نضيف مستمع حدث باستخدام [addEventListener](https://developer.mozilla.org/docs/Web/API/EventTarget/addEventListener)، وتوفير وظيفة للتنفيذ.

> **NOTE:** من الجدير بالذكر أن هناك طرقًا عديدة لإنشاء مستمعي الأحداث. يمكنك استخدام وظائف مجهولة، أو إنشاء وظائف مسماة. يمكنك استخدام اختصارات مختلفة، مثل تعيين خاصية `click`، أو استخدام `addEventListener`. في تمريننا، سنركز على `addEventListener` والوظائف المجهولة، لأنها التقنية الأكثر شيوعًا التي يستخدمها مطورو الويب. كما أنها الأكثر مرونة، حيث يعمل `addEventListener` لجميع الأحداث، ويمكن توفير اسم الحدث كمعامل.

### الأحداث الشائعة

هناك [عشرات الأحداث](https://developer.mozilla.org/docs/Web/Events) المتاحة للاستماع إليها عند إنشاء تطبيق. في الأساس، أي شيء يفعله المستخدم على الصفحة يثير حدثًا، مما يمنحك الكثير من القوة لضمان حصولهم على التجربة التي ترغب بها. لحسن الحظ، عادةً ما تحتاج فقط إلى عدد قليل من الأحداث. إليك بعض الأحداث الشائعة (بما في ذلك الاثنين اللذين سنستخدمهما عند إنشاء لعبتنا):
...
سنقوم بإنشاء لعبة لاستكشاف كيفية عمل الأحداث في JavaScript. لعبتنا ستختبر مهارة الكتابة لدى اللاعب، وهي واحدة من أكثر المهارات التي يجب أن يمتلكها جميع المطورين. يجب علينا جميعًا ممارسة الكتابة! تدفق اللعبة العام سيكون كالتالي:

- ينقر اللاعب على زر البدء ويتم تقديم اقتباس له ليكتبه
- يكتب اللاعب الاقتباس بأسرع ما يمكن في مربع النص
  - مع اكتمال كل كلمة، يتم تمييز الكلمة التالية
  - إذا ارتكب اللاعب خطأً، يتم تحديث مربع النص ليصبح أحمر
  - عندما يكمل اللاعب الاقتباس، يتم عرض رسالة نجاح مع الوقت المستغرق

لنبدأ ببناء لعبتنا، ونتعلم عن الأحداث!

### هيكل الملفات

سنحتاج إلى ثلاثة ملفات إجمالاً: **index.html**، **script.js** و **style.css**. لنبدأ بإعدادها لتسهيل الأمور علينا.
...
إذا استعرضنا المتطلبات، نعلم أننا سنحتاج إلى مجموعة من العناصر في صفحة HTML الخاصة بنا. هذا يشبه إلى حد ما وصفة، حيث نحتاج إلى بعض المكونات:
...
كل من هذه العناصر سيحتاج إلى معرفات حتى نتمكن من العمل معها في JavaScript الخاص بنا. سنضيف أيضًا مراجع لملفات CSS و JavaScript التي سنقوم بإنشائها.
...
سنحتاج إلى بعض العناصر لتسهيل البرمجة. مرة أخرى، مشابهة للوصفة، إليك ما سنحتاجه:
...
> **NOTE:** يمكننا استرداد العناصر وقتما نريد في الكود باستخدام `document.getElementById`. نظرًا لأننا سنشير إلى هذه العناصر بشكل منتظم، سنقوم بتجنب الأخطاء الإملائية مع النصوص الحرفية باستخدام الثوابت. يمكن أن تساعدك أطر العمل مثل [Vue.js](https://vuejs.org/) أو [React](https://reactjs.org/) في إدارة مركزية الكود بشكل أفضل.
...
- إعداد تتبع الكلمات
  ...
  - نقوم بتحويل `quote` إلى مصفوفة من `words` حتى نتمكن من تتبع الكلمة التي يكتبها اللاعب حاليًا
  - يتم تعيين `wordIndex` إلى 0، حيث سيبدأ اللاعب بالكلمة الأولى
- إعداد واجهة المستخدم
  - إنشاء مصفوفة من `spanWords`، تحتوي على كل كلمة داخل عنصر `span`
    - هذا سيسمح لنا بتمييز الكلمة على العرض
  - `join` المصفوفة لإنشاء سلسلة يمكننا استخدامها لتحديث `innerHTML` على `quoteElement`
...
عندما يكتب اللاعب، سيتم رفع حدث `input`. مستمع الحدث هذا سيتحقق من أن اللاعب يكتب الكلمة بشكل صحيح، ويتعامل مع حالة اللعبة الحالية. بالعودة إلى **script.js**، أضف الكود التالي إلى النهاية. سنقوم بتفصيله بعد ذلك.
...
- الاقتباس مكتمل، يُشار إليه بـ `typedValue` الذي يساوي `currentWord`، و `wordIndex` الذي يساوي واحدًا أقل من `length` لـ `words`
  ...
  - تقسيم `elapsedTime` على 1,000 لتحويله من ميلي ثانية إلى ثوانٍ
  - عرض رسالة نجاح
- الكلمة مكتملة، يُشار إليها بـ `typedValue` الذي ينتهي بمسافة (نهاية الكلمة) و `typedValue` الذي يساوي `currentWord`
  ...
  - تعيين `className` للكلمة الحالية إلى `highlight` للإشارة إليها كالكلمة التالية للكتابة
- الكلمة مكتوبة بشكل صحيح حاليًا (لكنها ليست مكتملة)، يُشار إليها بـ `currentWord` الذي يبدأ بـ `typedValue`
...
**الوصف:** قم بتوسيع لعبة الكتابة من خلال تنفيذ نظام صعوبة يتكيف مع أداء اللاعب. هذا التحدي سيساعدك على ممارسة التعامل مع الأحداث المتقدمة، تحليل البيانات، وتحديث واجهة المستخدم الديناميكية.

**المهمة:** قم بإنشاء نظام تعديل الصعوبة للعبة الكتابة بحيث:
1. يتتبع سرعة الكتابة لدى اللاعب (كلمات في الدقيقة) ونسبة الدقة
2. يتكيف تلقائيًا مع ثلاثة مستويات صعوبة: سهل (اقتباسات بسيطة)، متوسط (اقتباسات حالية)، صعب (اقتباسات معقدة تحتوي على علامات ترقيم)
3. يعرض مستوى الصعوبة الحالي وإحصائيات اللاعب على واجهة المستخدم
4. ينفذ عداد سلسلة نجاح يزيد الصعوبة بعد 3 أداءات جيدة متتالية
5. يضيف ردود فعل بصرية (ألوان، حركات) للإشارة إلى تغييرات الصعوبة

قم بإضافة العناصر اللازمة في HTML، أنماط CSS، ووظائف JavaScript لتنفيذ هذه الميزة. تأكد من التعامل مع الأخطاء بشكل صحيح وضمان أن اللعبة تبقى متاحة باستخدام تسميات ARIA المناسبة.
...
- عرض مربع حوار (modal) يحتوي على رسالة النجاح
