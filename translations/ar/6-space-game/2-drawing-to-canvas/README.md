...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-10-20T20:39:17+00:00",
...
✅ اقرأ [المزيد عن واجهة برمجة تطبيقات اللوحة](https://developer.mozilla.org/docs/Web/API/Canvas_API) على موقع MDN.
...
- `id`: قم بتعيينه للحصول على مرجع عندما تحتاج إلى التفاعل معه.
...
2. **الحصول على مرجع** لعنصر السياق الذي يجلس على عنصر اللوحة.
3. **تنفيذ عملية الرسم** باستخدام عنصر السياق.
...
افتح نسخة من هذا المجلد في Visual Studio Code. تحتاج إلى إعداد بيئة تطوير محلية، ويفضل أن تكون باستخدام Visual Studio Code مع NPM وNode مثبتين. إذا لم يكن لديك `npm` مثبتًا على جهاز الكمبيوتر الخاص بك، [إليك كيفية القيام بذلك](https://www.npmjs.com/get-npm).
...
1. **رسم** لوحة بخلفية سوداء  
   > نصيحة: أضف سطرين تحت التعليق المناسب في `/app.js`، وقم بتعيين عنصر `ctx` ليكون أسودًا، وحدد إحداثيات الأعلى/اليسار عند 0,0 والارتفاع والعرض ليكونا مساويين لعرض وارتفاع اللوحة.
2. **تحميل** القوام  
...
3. **رسم** البطل في وسط الشاشة في النصف السفلي  
   > نصيحة: استخدم واجهة برمجة التطبيقات `drawImage` لرسم heroImg على الشاشة، مع تحديد `canvas.width / 2 - 45` و`canvas.height - canvas.height / 4)`.
4. **رسم** 5*5 وحوش  
...
يرجى محاولة حلها بنفسك أولاً، ولكن إذا واجهت صعوبة، ألقِ نظرة على [الحل](../../../../6-space-game/2-drawing-to-canvas/solution/app.js)
...
لقد تعلمت كيفية الرسم باستخدام واجهة برمجة التطبيقات للوحة ثنائية الأبعاد؛ ألقِ نظرة على [واجهة برمجة تطبيقات WebGL](https://developer.mozilla.org/docs/Web/API/WebGL_API)، وحاول رسم كائن ثلاثي الأبعاد.
