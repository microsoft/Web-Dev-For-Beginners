...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-11-06T13:48:00+00:00",
...
- **Підключає** CSS-стилі у `<head>` для стилізації
...
- **Слугує** вашими файлами з локального URL (зазвичай `localhost:5500`)
...
| Область повідомлень | `message` | Відображає оновлення статусу |
...
- **Ініціалізує** змінні для відстеження за допомогою `let`, оскільки ці значення будуть оновлюватися під час гри
...
- **Створює** масив елементів `<span>`, обгортаючи кожне слово для індивідуального стилю
- **Об’єднує** елементи span в один рядок для ефективного оновлення DOM
- **Виділяє** перше слово, додаючи клас CSS `highlight`
...
- **Дозволяє** точний розрахунок швидкості набору тексту та часу завершення
...
- **Оновлює** візуальне виділення, видаляючи всі класи та виділяючи нове слово
...
1. Відстежує швидкість набору тексту гравця (слова за хвилину) і відсоток точності
...
3. Відображає поточний рівень складності та статистику гравця в інтерфейсі
...
|---------|-------------|------------------------|
| **Контроль введення** | Вимкніть обробник подій `input` після завершення та увімкніть його, коли натиснута кнопка | Управління подіями та контроль стану |
...
| **Система високих балів** | Зберігайте високі бали за допомогою `localStorage` | API зберігання браузера та збереження даних |

**Поради щодо реалізації:**
- **Досліджуйте** `localStorage.setItem()` і `localStorage.getItem()` для постійного зберігання
- **Практикуйте** додавання та видалення обробників подій динамічно
...
- [ ] Реалізуйте систему високих балів `localStorage` із секції викликів
- [ ] Створіть калькулятор слів за хвилину, який відображається після кожної гри
